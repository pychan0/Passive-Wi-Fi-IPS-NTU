...
sid_dict = {} #Session ID and RPI ID Mapping
...
async def Update_White_list():
    ...
    while True:
        if not opt.EnablePositionFilter:
            try:
                ...
                pass
        elif opt.EnablePositionFilter:
            try:
                print('[INFO] Dumping BW16 APs to Sniffers')
                await sio.emit('Update_WHITE_list',  {'WHITE_list': list(WHITE_list)},)
                await asyncio.sleep(opt.save_time)
            except:
                pass                 
...
async def RSSI_mesg(sid, data):
    try:
        ...
        Seq = data[RPI][2]
        channel = data[RPI][3]
        if Position_DATA['X'] is None and opt.EnablePositionFilter:
            return data
...
async def _Change_channel():
    if opt.EnableWhiteList and not opt.EnablePositionFilter:
        while True:
            channel = AP_CHANNEL
...
async def Show_RSSI_Data():
    ...
    if len(sid_dict) >= opt.monitors:
        ...
        save_tmp_RSSI_DATA = copy.deepcopy(RSSI_DATA)
        RSSI_DATA.clear()
        for channel in [CHANNEL_BW16_ROBOT]:
            print('save', channel)
            filepath = 'Channel_%d.csv' % channel
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_time", type=int, default=6, help="Duration time per saving cycle.")             
    parser.add_argument("--short_save_time", type=int, default=1, help="Check duration time per saving cycle.")    
    parser.add_argument("--rp_samples", type=int, default=200, help="Number of collections in recieving points.")    
    ...
    parser.add_argument("--RSSI_time_delay", type=float, default=0.7, help="Time delay for the same MAC.")            
    ...
    parser.add_argument("--EnablePositionFilter", type=bool, default=True, help="Enable Position Filter.(only in collecting mode)")    
